...
import {asyncAddTask} from "../redux/asyncFunctions/addTask"
...
export default function Main({ tasks }) {
    ...
    const completedFunction = (task) => {
        task.classList.toggle(styles1.taskCompleted);
    }
                ...
                <div>
                    {data.length ?
                        transitions((styles, item) => 
                            <animated.div className={styles1.task} style={styles} onDoubleClick={(e) => completedFunction(e.target)}>
                                <h2>{item.title}</h2>
                ...
                >
                    {
                        styles => (
                            <animated.div style={styles} className={styles1.modalBlock}>
                                <div>
                                    <input type="text" onChange={(e) => taskValue.current = e.target.value}/>
                                </div>
